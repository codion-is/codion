...
An {url-javadoc}{framework-db-core}/is/codion/framework/db/EntityConnection.html[EntityConnection] built via one of the builders below manages itself, regardless of protocol (JDBC, RMI, HTTP): it connects on demand, validates the underlying connection before each operation and re-establishes it when it has gone bad.
...
TIP: Use {url-javadoc}{framework-db-core}/is/codion/framework/db/EntityConnection.html#builder()[EntityConnection.builder()] to build a connection of the type specified by the {url-javadoc}{framework-db-core}/is/codion/framework/db/EntityConnection.html#CLIENT_CONNECTION_TYPE[CLIENT_CONNECTION_TYPE] configuration value, instead of naming a transport at compile time.
...
You can override the default description using the configuration property {url-javadoc}{framework-db-core}/is/codion/framework/db/EntityConnection.html#DESCRIPTION[EntityConnection.DESCRIPTION]
