...
Each entity type is defined by calling {url-javadoc}/is/codion/framework/domain/Domain.html#define-is.codion.framework.domain.entity.EntityId-is.codion.framework.domain.property.Property.Builder...-[Domain.define]. The framework assumes the *entityId* name contains the table name, unless the *tableName* parameter is specified.
