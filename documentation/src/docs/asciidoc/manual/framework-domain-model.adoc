...
{url-javadoc}/is/codion/framework/domain/property/ForeignKeyProperty.html[ForeignKeyProperty] is a property used to represent a foreign key relation. These foreign keys refer to the primary key of the referenced entity and must be constructed accordingly in case of composite primary keys.
...
foreignKeyProperty(Detail.MASTER_FK, "Master", Master.TYPE,
        asList(Detail.MASTER_ID_1, Detail.MASTER_ID_2));
----
