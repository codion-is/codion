...
{url-javadoc}/is/codion/framework/domain/property/SubqueryProperty.html[SubqueryProperty] is used to represent a property which gets its value from a subquery returning a single value. Note that in the example below *reference_id* must be available when the query is run, that is, the entity must include that column as a property.
