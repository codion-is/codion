...
Each column in a table is associated with a {url-javadoc}/is/codion/framework/domain/attribute/Attribute.html[Attribute] constant, which has a name and denotes the value type. The {url-javadoc}/is/codion/framework/domain/property/Attributes.html[Attributes] class provides factory methods for constructing Attribute objects and is usually statically imported, as will be assumed in the following examples.
...
Each entity type is defined by calling {url-javadoc}/is/codion/framework/domain/Domain.html#define-is.codion.framework.domain.entity.Entity.Identity-is.codion.framework.domain.property.Property.Builder...-[Domain.define]. The framework assumes the *entityId* name contains the table name, unless the *tableName* parameter is specified.
