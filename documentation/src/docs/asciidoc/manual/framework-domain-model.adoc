...
The *Entity* class is a map like structure, which maps column values to the attribute representing each column. Each table is represented by a entity type, identified by a EntityType constant with a name unique within the domain.
...
String constants for the Entities and Attribute constants for the properties in the World demo, here the *entityTypes* have a *T_* prefix (for *Type* or *Table*) whereas the *attributes* are prefixed by their respective table names.

[source,java]
----
include::{dir-world-source}/is/codion/framework/demos/world/domain/World.java[tags=entityTypesAndAttributes]
----
...
Each entity type is defined by calling {url-javadoc}/is/codion/framework/domain/Domain.html#define-is.codion.framework.domain.entity.EntityType-is.codion.framework.domain.property.Property.Builder...-[Domain.define]. The framework assumes the *entityType* name contains the table name, unless the *tableName* parameter is specified.
