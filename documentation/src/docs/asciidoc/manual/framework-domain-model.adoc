...
Each attribute is represented by the {url-javadoc}{framework-domain}/is/codion/framework/domain/entity/attribute/AttributeDefinition.html[AttributeDefinition] class or one of its subclasses, which encapsulate the metadata required for presenting and persisting the associated value.

A {url-javadoc}{framework-domain}/is/codion/framework/domain/entity/attribute/AttributeDefinition.html[AttributeDefinition] can be transient or based on table columns.
...
In case of composite primary keys you create a ColumnDefinition and specify the primary key index.
...
{url-javadoc}{framework-domain}/is/codion/framework/domain/entity/attribute/ColumnDefinition.html[ColumnDefinition] is used to represent attributes that are based on table columns.

[source,java,indent=0]
----
include::{dir-world-source}/is/codion/framework/demos/world/domain/WorldImpl.java[tags=columnAttribute]
----

==== Subquery

A ColumnDefinition can represent a subquery returning a single value.
...
{url-javadoc}{framework-domain}/is/codion/framework/domain/entity/attribute/BlobColumnDefinition.html[BlobColumnDefinition] is a specific ColumnDefinition for the Types.BLOB type which provides lazy loading for BLOB values. Using a standard ColumnDefinition with a byte array type works fine, but for lazy loading you have to use BlobColumnDefinition.
...
{url-javadoc}{framework-domain}/is/codion/framework/domain/entity/attribute/ForeignKeyDefinition.html[ForeignKeyDefinition] is an attribute used to represent a foreign key relation.
...
For databases supporting *Types.BOOLEAN* you simply use *ColumnDefinition*.
...
For databases lacking native boolean support we use the *Column.bool()* method, specifying the underlying type and the true/false values.

[source,java]
----
Customer.IS_ACTIVE.bool("Is active", Integer.class, 1, 0)
----

[source,java]
----
Customer.IS_ACTIVE.bool("Is active", String.class, "true", "false")
----

[source,java]
----
Customer.IS_ACTIVE.bool("Is active", Character.class, 'T', 'F')
----
...
=== DenormalizedAttributeDefinition
...
=== TransientAttributeDefinition

{url-javadoc}{framework-domain}/is/codion/framework/domain/entity/attribute/TransientAttributeDefinition.html[TransientAttributeDefinition] is used to represent an attribute which is not based on an underlying column, these attributes all have a default value of null and can be set and retrieved just like normal attributes.

=== DerivedAttributeDefinition

{url-javadoc}{framework-domain}/is/codion/framework/domain/entity/attribute/DerivedAttributeDefinition.html[DerivedAttributeDefinition] is used to represent a value which is derived from one or more attributes in the same entity. The value of a derived attribute is provided via a {url-javadoc}{framework-domain}/is/codion/framework/domain/entity/attribute/DerivedAttribute.Provider.html[DerivedAttribute.Provider] implementation as shown below.
