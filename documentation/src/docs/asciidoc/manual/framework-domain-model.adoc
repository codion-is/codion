...
When using the HTTP connection in an application using a custom data type, you must provide a JSON serializer for the type.

[source,java,indent=0]
----
include::{dir-world-source}/is/codion/framework/demos/world/domain/api/World.java[tags=customSerializer]
----

This method should be called in the domain model implementation, for the server side serialization, and in the application, before the connection to the server is established. See <<{dir-tutorials}/world.adoc#_domain, World domain model>> and <<{dir-tutorials}/world.adoc#_main_application_panel, WorldApplicationPanel>> constructor.
