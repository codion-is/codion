...
{url-javadoc}{framework-domain}/is/codion/framework/domain/entity/ColorProvider.html[ColorProvider] is used to provide colors for entity properties, used as background color in table cells for example.
...
* {url-javadoc}{framework-domain-test}/is/codion/framework/domain/entity/test/EntityTestUnit.html#initializeReferenceEntity{opar}is.codion.framework.domain.entity.EntityType{comma}java.util.Map{cpar}[initializeReferenceEntity] should return an instance of the given entity type to use for a foreign key reference required for inserting the entity being tested.
