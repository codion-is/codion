...
Each entity type is defined by calling {url-javadoc}{framework-domain}/is/codion/framework/domain/DefaultDomain.html#define{opar}is.codion.framework.domain.entity.EntityType{comma}is.codion.framework.domain.property.Property.Builder\...{cpar}[DefaultDomain.define]. The framework assumes the *entityType* name contains the table name, unless the *tableName* parameter is specified.
