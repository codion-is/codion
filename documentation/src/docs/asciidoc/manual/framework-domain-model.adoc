...
A column can be specified as being lazy, which means its value is not selected by default.
Note that the selected entity contains a null value by default for lazy loaded values.

[source,java,indent=0]
----
include::{dir-world-source}/is/codion/framework/demos/world/domain/WorldImpl.java[tags=lazy]
----
