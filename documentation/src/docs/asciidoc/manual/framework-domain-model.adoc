...
A Codion domain model is based around the {url-javadoc}{framework-domain}/is/codion/framework/domain/entity/Entity.html[Entity] interface, which represents a row in a table, providing access to the column values via its *get()* and *set()* methods.
...
NOTE: https://en.wikipedia.org/wiki/Constant_interface[Constant interfaces] are considered an antipattern in Java, a *public static final class* can be used instead in case interfaces is not acceptable, but that involves a bit more typing, since the constants must all be *public static final*.
