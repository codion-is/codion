...
interface Entity {
    ...
    +<T> T put(Column<T> attribute, T value)
}
-------------------------------------------
...
*{url-javadoc}{framework-domain}/is/codion/framework/domain/entity/Column.html[Column]*::
An Attribute subclass representing an actual column.
...
* Use the *EntityTypes* to create {url-javadoc}{framework-domain}/is/codion/framework/domain/entity/Column.html[Column] constants for each column and a {url-javadoc}{framework-domain}/is/codion/framework/domain/entity/ForeignKey.html[ForeignKey] constant for each foreign key.
...
The underlying column name is typically used as the *Column* name, but as with the *EntityType* you can use whatever value you want and specify the column name via the *columnName()* method when defining the associated *Property*.
...
_src/main/resources/META-INF/services/is.codion.framework.json.domain.EntityObjectMapperFactory_

[source]
----
include::{dir-world-resources}/META-INF/services/is.codion.framework.json.domain.EntityObjectMapperFactory[]
----
