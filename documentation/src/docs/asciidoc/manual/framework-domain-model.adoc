...
NOTE: https://en.wikipedia.org/wiki/Constant_interface[Constant interfaces] are considered an anti-pattern in Java, a *public static final class* can be used instead in case interfaces is not acceptable, but that involves a bit more typing, since the constants must all be *public static final*.
