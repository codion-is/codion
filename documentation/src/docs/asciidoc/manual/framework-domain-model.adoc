...
The domain model in Codion is based on the {url-javadoc}{framework-domain}/is/codion/framework/domain/entity/Entity.html[Entity] interface, which represents a row in a table. Defining the domain model involves creating constants representing the domain ({url-javadoc}{framework-domain}/is/codion/framework/domain/DomainType.html[DomainType]), the table ({url-javadoc}{framework-domain}/is/codion/framework/domain/entity/EntityType.html[EntityType]), columns ({url-javadoc}{framework-domain}/is/codion/framework/domain/entity/Attribute.html[Attribute]) and foreign keys ({url-javadoc}{framework-domain}/is/codion/framework/domain/entity/ForeignKey.html[ForeignKey]). These constants represent the domain API and are used when referring to tables, columns or foreign keys. These are wrapped in an interface denoting the entity type, for a convenient namespace.
...
The domain model is then implemented by extending the {url-javadoc}{framework-domain}/is/codion/framework/domain/DefaultDomain.html[DefaultDomain] class, which we use to define the domain entities and their properties based on the aformentioned API constants.
...
The domain model provides a {url-javadoc}{framework-domain}/is/codion/framework/domain/entity/Entities.html[Entities] instance via {url-javadoc}{framework-domain}/is/codion/framework/domain/Domain.html#getEntities{opar}{cpar}[getEntities()], which serves as a container for the domain entity definitions and as a factory for *Entity* and *Key* instances.
...
The {url-javadoc}{framework-domain}/is/codion/framework/domain/entity/Entity.html[Entity] class is a map like structure, which maps column values to the attribute representing each column. Each table is represented by a EntityType constant with a name unique within the domain.
