...
The *EntityType* and *Attribute* constants provide methods returning builders which allow for further configuration (such as nullability and maximum length for values and the caption and primary key generator for the entity definition). These factory methods are usually statically imported, but the class names are included here for clarity.
