...
A Codion domain model is based on the {url-javadoc}{framework-domain}/is/codion/framework/domain/entity/Entity.html[Entity] interface, which represents a row in a table, providing access to the column values via its *get()* and *put()* methods.
...
To define a domain model start by creating constants representing the following:

[cols="1,4"]
|===
|Domain|{url-javadoc}{framework-domain}/is/codion/framework/domain/DomainType.html[DomainType]
|Table|{url-javadoc}{framework-domain}/is/codion/framework/domain/entity/EntityType.html[EntityType]
|Column|{url-javadoc}{framework-domain}/is/codion/framework/domain/entity/Attribute.html[Attribute]
|ForeignKey|{url-javadoc}{framework-domain}/is/codion/framework/domain/entity/ForeignKey.html[ForeignKey]
|===
...
The domain model is implemented by extending the {url-javadoc}{framework-domain}/is/codion/framework/domain/DefaultDomain.html[DefaultDomain] class, where we define the domain entities and their properties based on the aformentioned API constants and add them to the domain.

NOTE: The domain model implementation must be on the classpath when running with a local JDBC connection, but when running with a RMI or HTTP connection the domain model API is sufficient. If you forsee using RMI or HTTP connections it is recommended to split your domain model into separate API and implementation modules, to simplify client configurations (see https://github.com/bjorndarri/chinook[Chinook] and https://github.com/bjorndarri/world[World] demo applications).
...
The {url-javadoc}{framework-domain}/is/codion/framework/domain/entity/Entity.html[Entity] class is a map like structure, mapping column values to the attribute representing each column. Each table is represented by a EntityType constant with a name unique within the domain.

NOTE: Typically, the *entityType* name contains the underlying table name, but you can use whatever identifying string you want and specify the table name via the *tableName()* builder method when defining the entity.

== Attributes

Each column in a table is associated with a {url-javadoc}{framework-domain}/is/codion/framework/domain/entity/Attribute.html[Attribute] constant, which has a name and denotes the value type. The {url-javadoc}{framework-domain}/is/codion/framework/domain/entity/EntityType.html[EntityType] class provides factory methods for constructing Attribute instances for that entity type.

NOTE: Typically, the *attribute* name is the underlying column name, but as with the *entityType* you can use whatever value you want and specify the column name via the *columnName()* method when defining the associated Property.
...
Each foreign key of a table is associated with {url-javadoc}{framework-domain}/is/codion/framework/domain/entity/ForeignKey.html[ForeignKey] constant, which has a name and specifies which column attributes the foreign key is based on.
