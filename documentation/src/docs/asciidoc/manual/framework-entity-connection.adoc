...
or on a connection instance via {url-javadoc}{framework-db-local}/is/codion/framework/db/local/LocalEntityConnection.html#setLimitForeignKeyReferenceDepth{opar}boolean{cpar}[setLimitForeignKeyReferenceDepth()]
