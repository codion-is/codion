...
{url-entity-connection}#select{opar}java.util.List{cpar}[select(List<Key> keys)]
...
{url-entity-connection}#selectSingle{opar}is.codion.framework.domain.entity.Key{cpar}[selectSingle(Key key)]
