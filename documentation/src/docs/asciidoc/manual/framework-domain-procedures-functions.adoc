...
Procedures and functions are executed via {url-javadoc}{framework-db}/is/codion/framework/db/EntityConnection.html#execute(is.codion.common.db.operation.FunctionType,P)[EntityConnection.execute()]. The connection is passed to the implementation, which can use it for database operations.
