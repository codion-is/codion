...
The model layer is a complete, UI-independent application: data retrieval, editing, validation, selection, master-detail coordination and all associated state live here, exposed through the framework's <<manual.adoc#_reactive_classes, reactive classes>>.
...
An {url-javadoc}{framework-model}/is/codion/framework/model/EntityModel.html[EntityModel] is the composition root for a single entity type: an <<manual.adoc#_entityeditmodel, edit model>> (wrapping the <<manual.adoc#_entityeditor, editor>> — the write path), usually a <<manual.adoc#_entitytablemodel, table model>> (with its <<manual.adoc#_entityquerymodel, query model>> — the read path), and any detail models.
...
See <<manual.adoc#_model_linking, Model linking>> for the linking configuration — what happens on selection, insert, update and delete.
...
NOTE: Since models are UI-free, they are constructed and exercised directly in unit tests — create the model with a test connection, edit, insert, assert.
