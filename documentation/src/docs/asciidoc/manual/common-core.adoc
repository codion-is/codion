...
The {url-javadoc}{common-core}/is/codion/common/event/Event.html[Event] class is a synchronous event implementation used throughout the framework. Classes typically expose their events via public *event* accessors. Events are triggered by calling the *run* method in case no data is associated with the event or *accept* in case data should be propogated to listeners.
