...
Databases sometimes prefix view names with `VW_`, `V_`, or similar. Setting this prefix removes it from generated entity type names.
...
Without prefix:: `EntityType TYPE = DOMAIN.entityType("vw_orders");`
With prefix "VW_":: `EntityType TYPE = DOMAIN.entityType("orders");`

NOTE: Both view prefix and view suffix can be used simultaneously. The prefix is removed first, then the suffix.
