...
[source]
----
Observer<T>                       (root reactive interface)
├── AbstractObserver<T>           (canonical implementation)
│   ├── Value<T>                  (mutable observable)
│   ├── State                     (mutable boolean state)
│   └── Event<T>                  (push-only notifications)
├── Observable<T>                 (read-only value wrapper)
└── ObservableState               (read-only boolean state)
----

This hierarchy enables type-based discovery: any type that extends or implements `Observer` is reactive by definition. The concrete types (`Value`, `State`, `Event`) extend `AbstractObserver` for their implementation, while the read-only interfaces (`Observable`, `ObservableState`) delegate to an underlying observer.

=== Observer Interface

The {url-javadoc}{common-reactive}/is/codion/common/reactive/observer/Observer.html[Observer] interface is the foundation for all change notification:
...
public interface Observer<T> {
    ...
    // The underlying observer for delegation
    Observer<T> observer();
}
----
...
2. **Weak reference support** for memory-safe UI bindings

3. **Conditional observers** via `when()` for value-specific or predicate-based filtering

4. **Delegation pattern** via `observer()` - all listener methods are default implementations that delegate to `observer()`, enabling two implementation strategies:
   - Extend {url-javadoc}{common-reactive}/is/codion/common/reactive/observer/AbstractObserver.html[AbstractObserver] where `observer()` returns `this`
   - Implement `observer()` to return a delegate (useful for wrapper types)

=== AbstractObserver

{url-javadoc}{common-reactive}/is/codion/common/reactive/observer/AbstractObserver.html[AbstractObserver] is the canonical implementation of Observer. It provides thread-safe listener management and is the base class for `Value`, `State`, and `Event`.
...
public abstract class AbstractObserver<T> implements Observer<T> {
    ...
    public final Observer<T> observer() {
        return this;  // Default methods resolve to concrete implementations
    }

    // Subclasses trigger notifications by calling:
    protected final void notifyListeners(@Nullable T data) { ... }
}
----

Since `observer()` returns `this`, the default methods in `Observer` delegate back to the concrete implementations in `AbstractObserver`, avoiding any recursion.

=== Observable Interface

{url-javadoc}{common-reactive}/is/codion/common/reactive/observer/Observable.html[Observable] combines value access with observation:
...
Observable inherits all listener methods from Observer. This allows exposing read-only observables while keeping mutation control private. For example, a class can expose an `Observable<T>` publicly while keeping the underlying `Value<T>` private.

== Value Implementation

=== Value Interface

{url-javadoc}{common-reactive}/is/codion/common/reactive/value/Value.html[Value] is the primary mutable observable:
...
{url-javadoc}{common-reactive}/is/codion/common/reactive/state/State.html[State] is optimized for boolean values:
...
{url-javadoc}{common-reactive}/is/codion/common/reactive/event/Event.html[Event] provides push-only notifications:
