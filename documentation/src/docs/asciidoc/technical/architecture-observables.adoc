...
3. **Boolean return values**: Indicate whether the observer was modified
...
public interface Value<T> extends Observable<T> {
    enum Notify {
        WHEN_SET,     // Notify on every set() call
        WHEN_CHANGED  // Notify only when value changes
    }
...
button.setEnabled(processing.not());  // Disabled while processing
...
[source,java]
----
// WHEN_CHANGED: Only when value changes (default)
Value<Integer> counter = Value.builder()
    .nonNull(0)
    .build();  // Uses WHEN_CHANGED by default

counter.set(1);  // Notifies
counter.set(1);  // No notification

// WHEN_SET: Always notify, even if value unchanged
...
status.set("OK");  // Still notifies with WHEN_SET
----
...
4. **Link values** instead of manual synchronization:
   ```java
   // Instead of:
