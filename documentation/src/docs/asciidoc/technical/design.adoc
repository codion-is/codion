...
Many classes expose their internal state via the {url-javadoc}{common-core}/is/codion/common/observer/Mutable.html[Mutable] or {url-javadoc}{common-core}/is/codion/common/value/Value.html[Value] classes, which can be used to mutate the associated state or observe it by adding listeners or consumers.

[source,java]
----
include::{dir-manual-source}/is/codion/framework/demos/manual/design/DesignAndStyle.java[tags=mutable,indent=0]
----
