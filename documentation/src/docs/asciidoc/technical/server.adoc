...
The Codion server does not perform any user authentication by default, it leaves that up the underlying database. An authentication layer can be added by implementing a {url-javadoc}{common-rmi}/is/codion/common/rmi/server/Authenticator.html[Authenticator] and registering it with the ServiceLoader.
...
----
codion.server.objectInputFilterFactoryClassName=\
    is.codion.common.rmi.server.WhitelistObjectInputFilterFactory
----
