...
An {url-javadoc}{common-rmi}/is/codion/common/rmi/server/Authenticator.html[Authenticator] swapping in a shared
database user leaves the database seeing that one user doing everyone's work. Anything relying on who the work
is really for — an audit trigger, a row level security policy, a DBA looking at which session is running a slow
query — then has nothing to go on.

The server addresses the reporting half of that on its own: each connection is stamped with the identity of the
client about to use it, via
{url-javadoc}{common-db}/is/codion/common/db/database/Database.html#clientInfo(java.sql.Connection,is.codion.common.db.database.ClientInfo)[Database.clientInfo()],
which carries the user, the client type and the client host as far as the database allows. What that means, and
what it costs, is dialect specific — Db2 and Oracle put the values in session registers readable from SQL and
send them along with the next statement, PostgreSQL has room for only an application name and sets it with a
statement of its own, and other databases have nowhere to put them at all, in which case nothing happens.
Disable it with `codion.db.clientInfo=false`.

[#_session_context]
==== SessionContext

Anything beyond identity — the value an audit trigger reads, the variable a row level security policy consults —
needs a {url-javadoc}{common-db}/is/codion/common/db/database/SessionContext.html[SessionContext], registered
with the ServiceLoader. The server applies the registered contexts to a connection before the client uses it and
removes them again afterwards, which with a connection pool means on every connection check out, so an
implementation is worth keeping cheap.

* Shared contexts are applied before ones naming a client type, and removed in reverse, so a client type specific
context sits on top of the general setup and comes off before it.
* Failing to apply a context fails the client's operation, rather than letting a query run against a connection
whose session state is not what the application expects.
* Failing to remove one discards the connection rather than handing it, and whatever is still set on it, to the
next client.

The interface lives in the `codion-common-db` module rather than with the server, so that implementing one costs
an application nothing it does not already have.

===== SessionContext examples

<<{dir-tutorials}/chinook/chinook.adoc#_session_context, ChinookSessionContext>>
...
The framework provides a way to configure a {url-jdk-javadoc}/java.base/java/io/ObjectInputFilter.html[ObjectInputFilter] for deserialization, by specifying a {url-javadoc}{common-rmi}/is/codion/common/rmi/server/ObjectInputFilterFactory.html[ObjectInputFilterFactory] implementation class with the following system property.
...
|CRUD|JSON|JSON
|{url-javadoc}{framework-db}/is/codion/framework/db/EntityConnection.html#execute(is.codion.framework.domain.operation.FunctionType)[Functions]|JSON|Java serialization
|{url-javadoc}{framework-db}/is/codion/framework/db/EntityConnection.html#execute(is.codion.framework.domain.operation.ProcedureType)[Procedures]|JSON|N/A
|{url-javadoc}{framework-db}/is/codion/framework/db/EntityConnection.html#report(is.codion.framework.domain.report.ReportType,P)[Reports]|JSON|Java serialization
|{url-javadoc}{framework-db}/is/codion/framework/db/EntityConnection.html#entities()[Entities]|N/A|Java serialization
|===

IMPORTANT: The hybrid approach protects the server from deserialization attacks while maintaining compatibility with complex return types like `JasperPrint` and `Entities` that don't support JSON serialization.
...
See the manual section on <<../manual/manual.adoc#_entityobjectmapperfactory, HTTP/JSON Serialization>> for details on registering parameter types via {url-javadoc}{framework-json-domain}/is/codion/framework/json/domain/EntityObjectMapperFactory.html[EntityObjectMapperFactory].
...
With `codion.server.rmi=false` the server serves HTTP only: no RMI objects are exported, no RMI registry
is created and a remote `connect()` is refused — the RMI ports simply do not exist.
The RMI admin interface is controlled independently; configuring an admin port exports the server for the
<<server-monitor.adoc#_server_monitor, ServerMonitor>>, with or without RMI client connections.
Disabling RMI is the recommended baseline for an internet-facing server, see
<<internet-deployment.adoc#_internet_deployment, Internet deployment>>.
...
Beyond the <<server-monitor.adoc#_server_monitor, ServerMonitor>>, the server can register its runtime metrics
as JMX MBeans on the platform MBean server, making them available to standard tooling — Prometheus and Grafana
via the https://github.com/prometheus/jmx_exporter[Prometheus JMX Exporter], as well as JConsole and VisualVM.
This runs alongside, not instead of, the Swing server monitor.
...
The JVM's own thread/GC/CPU/memory numbers are standard platform MXBeans, exported separately by the agent, so
they are not duplicated here. The MBeans are local to the JVM — nothing is exposed until the exporter agent is
attached, and no JMX remote connector is involved.

The `framework/server/src/main/monitoring` directory contains a ready-to-use JMX Exporter configuration
(`jmx_exporter.yaml`), a starter Grafana dashboard (`grafana-dashboard.json`), and a step-by-step guide
(`README.md`) covering the exporter agent, the Prometheus scrape config, the Grafana import and the full
metrics reference.
...
[source]
----
include::{dir-manual-source}/is/codion/manual/framework/server/ServerDemo.java[tags=runServer]
----
...
[source]
----
include::{dir-manual-source}/is/codion/manual/framework/server/ServerDemo.java[tags=runServerWithHttp]
----