...
The Codion server does not perform any user authentication by default, it leaves that up the underlying database. An authentication layer can be added by implementing a {url-javadoc}/is/codion/common/rmi/server/LoginProxy.html[LoginProxy] and setting the following system property.
...
A {url-javadoc}/is/codion/common/rmi/server/SerializationWhitelist.html[serialization whitelist] can be used by setting the following system property.
...
[source]
----
include::{dir-source}/src/main/config/codion_server.policy[]
----
====
