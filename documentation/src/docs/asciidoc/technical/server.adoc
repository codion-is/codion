...
The Codion server does not perform any user authentication by default, it leaves that up the the underlying database. An authentication layer can be added by implementing a {url-javadoc}/dev/codion/common/rmi/server/LoginProxy.html[LoginProxy] and setting the following system property.
