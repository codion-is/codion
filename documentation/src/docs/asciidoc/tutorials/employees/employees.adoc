...
The entityType in this example contains the actual table name (employees.department), but the table name can be specified via the __table()__ builder method when the entity is defined. The column names are the actual column names from the table, but these can also be specified via the __name()__ builder method.
...
Here we set the default order by, the *formatter* which is responsible for providing *toString()* implementations for entities, the *smallDataset* attribute which hints that it is OK to automatically base ComboBoxes on the entity, and we also set the *caption*.
...
Here we set the *generator* to *Generator.sequence("employees.employee_seq")* which, as the name suggests, fetches a value from a sequence.
Here we also introduce the <<{basedir}/manual/manual.adoc#_foreign_keys, ForeignKey>>, the *orderBy*, as well as a *CellColor* which is responsible for providing a custom color for an entity instance.
...
NOTE: The user credentials {url-javadoc}{framework-domain-test}/is/codion/framework/domain/test/DomainTest.html[DomainTest] runs with are specified via the *codion.test.user* system property.
...
For further information on parameters required for running Codion applications see <<{basedir}/technical/technical.adoc#_client_configuration, client configuration>> and <<{basedir}/technical/technical.adoc#_server_configuration, server configuration>>.
...
We must provide a constructor with a single *EntityConnection* parameter.
...
We create a {url-javadoc}{swing-framework-model}/is/codion/swing/framework/model/component/SwingEntityComboBoxModel.html[SwingEntityComboBoxModel] for the Employee.MANAGER_FK attribute, using the static *builder()* method, configured to only display employees with the job PRESIDENT or MANAGER.
...
See <<{basedir}/technical/technical.adoc#_client_configuration, client configuration>> for what configuration is required for running the client.
