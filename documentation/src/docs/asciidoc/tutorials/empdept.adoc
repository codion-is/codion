...
We start by creating a class named EmpDept in a package of our choosing, extending Domain and define the identity and attribute constants required for the department {url-javadoc}/is/codion/framework/domain/entity/Entity.html[Entity], which is based on the SCOTT.DEPT table. The identity constant defines the entity itself and will be referred to as the *entityType* (here prefixed with T_), the attributes represent the columns in the table. The entityType in this example contains the actual table name (scott.dept), but the table name can be specified via a __tableName__ parameter when the entity is defined. The attribute names are the actual column names from the table, but that is not required either.
...
In many cases we can use the default model implementations, but here we will customize the employee edit model by extending {url-javadoc}/is/codion/swing/framework/model/SwingEntityEditModel.html[SwingEntityEditModel].
...
We start by calling the super class method which creates an unfiltered {url-javadoc}/is/codion/framework/model/EntityComboBoxModel.html[EntityComboBoxModel], then we configure the one for the manager foreign key so it only display employees with the job PRESIDENT or MANAGER.
...
We override the *intializeUI* method to construct the actual UI, used for editing the department entity. The {url-javadoc}/is/codion/swing/framework/ui/EntityEditPanel.html[EntityEditPanel] class provides methods for creating all the basic controls required, named *create...*, such as *createTextField(attribute)* or *createForeignKeyComboBox(foreignKeyAttribute)*.
...
[source,java]
----
include::{dir-source}/is/codion/framework/demos/empdept/ui/DepartmentTablePanel.java[tags=getPrintControls]
----
...
[source,java]
----
include::{dir-source}/is/codion/framework/demos/empdept/ui/EmpDeptAppPanel.java[tags=getToolsControls]
----
...
[source,java]
----
include::{dir-source}/is/codion/framework/demos/empdept/ui/EmpDeptAppPanel.java[tags=initializeApplicationModel]
----
...
[source,java]
----
include::{dir-source}/is/codion/framework/demos/empdept/ui/EmpDeptAppPanel.java[tags=employeeModelBuilder]
----
...
[source,java]
----
include::{dir-source}/is/codion/framework/demos/empdept/ui/EmpDeptAppPanel.java[tags=employeePanelBuilder]
----
