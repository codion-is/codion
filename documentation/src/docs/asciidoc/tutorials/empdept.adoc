...
We start by calling the super class method which creates an unfiltered {url-javadoc}{framework-model}/is/codion/swing/framework/model/EntityComboBoxModel.html[EntityComboBoxModel], then we configure the one for the manager foreign key so that it only displays employees with the job PRESIDENT or MANAGER.
