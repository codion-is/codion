...
The Codion framework is based around the *Entity* class, a map-like structure, representing a row in a table. The domain is modelled much like an E/R diagram, with a *EntityIdentity* and *EntityDefinition* per table, an *Attribute* and a *Property* for each column and a *ForeignKeyProperty* for each foreign key relationship.
...
First we define a entity based on the table *store.customer*, using a *EntityIdentity* constant to identify the entity type (the *entityId*) and to provide attributes for all of its properties.
