...
Each table has an associated search panel, which can be displayed by using the button highlighted in the screenshot below. This button toggles through the avilable search panel views; *hidden*, *visible* and *advanced*.

image::help/search_cleared.png[Search, cleared]
...
When the search condition has been changed, as in, when it does not represent the data currently in the table, the refresh button in the lower left corner becomes enabled, indicating that a refresh is in order. Further column conditions can be added before a refresh is performed.

image::help/search_string_wildcard_no_refresh.png[Search, string wildcard, no refresh]

There are multiple ways of refreshing the table:

* Pressing the kbd:[Enter] key with the cursor somewhere on a column condition panel
* Using the aformentioned refresh button or its kbd:[F5] keyboard shortcut
...
NOTE: You can tell a column condition is enabled by its BOLD header and shaded data.

image::help/search_string_wildcard.png[Search, string wildcard]
...
image::help/search_string_wildcard_and_selection.png[Search, string wildcard and selection]

Enabling the *advanced* condition panel view in order to change the *Name* condition to *NOT EQUAL*.

image::help/search_string_wildcard_negated_and_selection.png[Search, string wildcard negated and selection]

Adding a *LESS THAN* condition to the *Price* column.

image::help/search_string_wildcard_negated_number_less_and_selection.png[Search, string wildcard negated, number less and selection]

Changing the *Price* condition to *GREATER THAN OR EQUAL*.

image::help/search_string_wildcard_negated_number_greater_or_equal_than_and_selection.png[Search, string wildcard negated, number greater or equal and selection]

Changing the *Price* condition to *BETWEEN* and the *Genre* condition to *NOT EQUAL*.

image::help/search_string_wildcard_negated_number_between_and_selection_negated.png[Search, string wildcard negated, number between and selection]

Adding a *NOT EQUAL* condition to the *Composer* column with an empty value.

image::help/search_string_wildcard_negated_number_between_selection_negated_and_not_null.png[Search, string wildcard negated, number between, selection and not null]

NOTE: When searching for empty values, the condition must be manually enabled, since its automatically enabled only when a non-empty value is entered.
