...
==== Copying Individual Columns

To copy the cell values for the selected column:

1. Select one or more rows, with the required column highlighted
2. Press kbd:[Ctrl + Alt + Shift + C] or use the *Copy* → *Copy column* action in the popup menu
3. Paste the values wherever needed
...
** Configure the column order, using kbd:[Alt + Up/Down] or the buttons
4. Click *Clipboard* to export to the clipboard or *To file* to select a destination file

You can save and open export column configurations, and access recently used configuration files in the drop down menu.
...
By default, column condition operators are set to *EQUAL* and foreign keys to *IN*, or *EQUAL* when the foreign key value is selected from a combo box, to change the operator you can use the kbd:[Ctrl + UP / DOWN] keyboard shortcut (the operator is displayed in a tooltip) or switch to the *advanced* search panel view, see below.

NOTE: On macOS, use kbd:[⌘] (Command) instead of kbd:[Ctrl] for keyboard shortcuts.

The *IN* and *NOT IN* fields collect a set of values, the value in the field counting as one of them.
Press kbd:[Insert] to add the value in the field to the set, clearing the field for the next one. kbd:[Enter] does the same in a text field, and in a search field once a search has found the value, kbd:[Enter] on an empty field refreshing the table as usual.
The button beside the field displays the number of values collected, and opens a list of them, also available via kbd:[Alt + &#x2193;], where kbd:[Delete] removes the selected values and *Clear* removes them all.
...
|kbd:[Ctrl + Alt + Shift + C]|Copy selected column cells to clipboard
...
|kbd:[Ctrl + Enter]|Enable/disable column condition
|kbd:[Ctrl + Shift + Enter]|Clear column condition
...
|kbd:[Ctrl + Enter or &#x2193;]|Select next
|kbd:[Shift + Enter or &#x2193;]|Add next to selection
|kbd:[&#x2191;]|Find previous
|kbd:[Ctrl + &#x2191;]|Select previous
|kbd:[Shift + &#x2191;]|Add previous to selection
|kbd:[Ctrl + Enter]|Toggle search result selection
