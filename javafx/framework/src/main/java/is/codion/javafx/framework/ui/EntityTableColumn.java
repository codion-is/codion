...
public final class EntityTableColumn<T> extends FXEntityListModel.AttributeTableColumn<T> {
  ...
  private PropertyConditionView<T> createConditionView(FXEntityListModel listModel, Property<T> property) {
    if (property instanceof ColumnProperty || property instanceof ForeignKeyProperty) {
      ColumnConditionModel<? extends Attribute<?>, T> conditionModel = (ColumnConditionModel<? extends Attribute<?>, T>)
              listModel.conditionModel().conditionModels().get(property.attribute());
