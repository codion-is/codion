...
In this tiny example we create a *CustomerPanel* by extending a *EntityPanel* and a *CustomerEditPanel* and *AddressEditPanel* by extending *EntityEditPanel*. The edit panel, as the name suggests, provides the UI for editing entity instances. In the *main* method we use these building blocks to assemble a client.
