...
:version: 0.17.16
:url-help: https://codion.is/doc/{version}/jdk11/help
:url-manual: https://codion.is/doc/{version}/jdk11/manual
:url-technical: https://codion.is/doc/{version}/jdk11/technical
...
I started developing Codion back in 2004, when the only Swing rich client CRUD framework available was practically unusable, and I was not about to abandon my good friend Swing in favor of my sworn enemy, HTML, which was fast becoming all the rage in the world of CRUD applications.
...
* Limit accidental complexity and be enjoyable to use.
...
In the below example, we define a domain model with two entities, a customer and an address, with a master/detail retionship, using the following steps:
...
. Use the *EntityType* to create *Attribute* constants for each column and the *ForeignKey*.
. Create two *Entity* definitions using properties based on the *Attribute* constants, and add them to the domain model.

NOTE: The *EntityType* and *Attribute* constants are wrapped in an interface for a convenient namespace and are used whenever we need to refer to the *Entity* and its values
