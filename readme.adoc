...
image:https://img.shields.io/maven-central/v/is.codion/codion-swing-framework-ui?label=stable&color=blue[link="https://central.sonatype.com/artifact/is.codion/codion-swing-framework-ui"]
...
Localized messages are available in English (default) and Icelandic. There are a whole lot of {url-technical}/technical.html#_internationalization_i18n[localized messages] so if you are interested in providing translations that would be much appreciated. This i18n page can be generated with the following Gradle target.
