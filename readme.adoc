...
My main motivation for developing Codion back in 2004 was the lack of Java application frameworks that were not based on the Java Enterprise Edition. I was writing rather basic desktop CRUD appliations, so I wanted to stick with Java Standard Edition components, Swing, JDBC and RMI.
...
. Create an *EntityDefinition* for each entity using the *EntityType* constant, based on *AttributeDefinitions* created using the *Column* and *ForeignKey* constants, and add the entity definitions to the domain model.
