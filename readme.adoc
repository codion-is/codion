...
Binaries for the current release *{version}* are available on Maven Central.

Snapshots of the current development version *{version-dev}-SNAPSHOT* are available in link:https://s01.oss.sonatype.org/content/repositories/snapshots[Sonatype's snapshots repository].
...
|===
|DBMS|Artifact
|Db2|is.codion:codion-dbms-db2database:{version}
|Derby|is.codion:codion-dbms-derby:{version}
|H2|is.codion:codion-dbms-h2database:{version}
