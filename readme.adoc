...
A release has yet to be published to Maven Central, but snapshots of the current development version *{version}-SNAPSHOT* are available in link:https://s01.oss.sonatype.org/content/repositories/snapshots[Sonatype's snapshots repository].
...
NOTE: The "waterfall" master/detail UI layout in these demo applications is simply what the framework provides by default and can be customized at will.
