...
public abstract class AbstractEntityTableModel<E extends EntityEditModel> implements EntityTableModel<E> {
	...
	private void onInsert(Collection<Entity> insertedEntities) {
		...
		if (!onInsert.isEqualTo(OnInsert.DO_NOTHING) && !entitiesToAdd.isEmpty()) {
			...
			switch (onInsert.get()) {
				case ADD_TOP:
					items().visible().addItemsAt(0, entitiesToAdd);
					break;
				case ADD_TOP_SORTED:
					if (items().visible().addItemsAt(0, entitiesToAdd)) {
						sort();
					}
					break;
				case ADD_BOTTOM:
					items().visible().addItemsAt(items().visible().count(), entitiesToAdd);
					break;
				case ADD_BOTTOM_SORTED:
					if (items().visible().addItemsAt(items().visible().count(), entitiesToAdd)) {
						sort();
					}
					break;
				default:
