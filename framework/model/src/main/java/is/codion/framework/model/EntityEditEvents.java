...
public final class EntityEditEvents {
   ...
   */
  public static void addInsertListener(final EntityType<? extends Entity> entityType, final EventDataListener<List<Entity>> listener) {
   ...
   * @param listener the listener
   */
  public static void addUpdateListener(final EntityType<? extends Entity> entityType, final EventDataListener<Map<Key, Entity>> listener) {
   ...
   * @param listener the listener
   */
  public static void addDeleteListener(final EntityType<? extends Entity> entityType, final EventDataListener<List<Entity>> listener) {
   ...
   */
  public static void removeInsertListener(final EntityType<? extends Entity> entityType, final EventDataListener<List<Entity>> listener) {
   ...
   * @param listener the listener to remove
   */
  public static void removeUpdateListener(final EntityType<? extends Entity> entityType, final EventDataListener<Map<Key, Entity>> listener) {
   ...
   * @param listener the listener to remove
   */
  public static void removeDeleteListener(final EntityType<? extends Entity> entityType, final EventDataListener<List<Entity>> listener) {
  ...
  private static final class EntityEditObserver {
    ...
    private final Map<EntityType<? extends Entity>, WeakObserver<List<Entity>>> insertEvents = new ConcurrentHashMap<>();
    private final Map<EntityType<? extends Entity>, WeakObserver<Map<Key, Entity>>> updateEvents = new ConcurrentHashMap<>();
    private final Map<EntityType<? extends Entity>, WeakObserver<List<Entity>>> deleteEvents = new ConcurrentHashMap<>();

    private void addInsertListener(final EntityType<? extends Entity> entityType, final EventDataListener<List<Entity>> listener) {
      getInsertObserver(entityType).addDataListener(listener);
    }

    private void removeInsertListener(final EntityType<? extends Entity> entityType, final EventDataListener<List<Entity>> listener) {
      getInsertObserver(entityType).removeDataListener(listener);
    }

    private void addUpdateListener(final EntityType<? extends Entity> entityType, final EventDataListener<Map<Key, Entity>> listener) {
      getUpdateObserver(entityType).addDataListener(listener);
    }

    private void removeUpdateListener(final EntityType<? extends Entity> entityType, final EventDataListener<Map<Key, Entity>> listener) {
      getUpdateObserver(entityType).removeDataListener(listener);
    }

    private void addDeleteListener(final EntityType<? extends Entity> entityType, final EventDataListener<List<Entity>> listener) {
      getDeleteObserver(entityType).addDataListener(listener);
    }

    private void removeDeleteListener(final EntityType<? extends Entity> entityType, final EventDataListener<List<Entity>> listener) {
    ...
    private WeakObserver<List<Entity>> getInsertObserver(final EntityType<? extends Entity> entityType) {
      return insertEvents.computeIfAbsent(requireNonNull(entityType), type -> new WeakObserver<>());
    }

    private WeakObserver<Map<Key, Entity>> getUpdateObserver(final EntityType<? extends Entity> entityType) {
      return updateEvents.computeIfAbsent(requireNonNull(entityType), type -> new WeakObserver<>());
    }

    private WeakObserver<List<Entity>> getDeleteObserver(final EntityType<? extends Entity> entityType) {
