...
//			}
		...
		private void out() {
			if (!result.isEmpty() /* darri added */ && result.charAt(result.length() - 1) == ',') {
				result.append(" ");
			}
			result.append(token);
		}
		...
		private static boolean isFunctionName(String tok) {
			if (tok == null || tok.isEmpty()) {
				...
				boolean isIdentifier = isJavaIdentifierStart(begin) || '"' == begin;
		...
		private void appendUntilToken(String stopToken) {
			StringBuilder quoted = new StringBuilder(this.token);
			String t;
			do {
				t = tokens.nextToken();
				quoted.append(t);
