...
final class MariaDBDatabase extends AbstractDatabase {
	...
	static {
		...
		ERROR_TYPES.put(4025, ErrorType.CHECK_CONSTRAINT);
	...
	/**
	 * The driver prefixes each message with the connection id, {@code (conn=42) ...}
	 */
	@Override
	protected String message(SQLException exception) {
		String message = exception.getMessage();
		if (message != null && message.startsWith(CONNECTION_PREFIX)) {
			int endIndex = message.indexOf(") ");

			return endIndex == -1 ? message : message.substring(endIndex + 2);
		}
