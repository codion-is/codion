...
final class FilterTableHeaderRenderer<R, C> implements TableCellRenderer {
	...
	public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected,
		...
		if (component instanceof JLabel) {
			...
			FilterTableColumn<C> tableColumn = filterTable.columnModel().getColumn(column);
			ConditionModel<?> filterModel = filterTable.model().filters().optional(tableColumn.identifier()).orElse(null);
	...
	private Icon sortArrowIcon(C identifier, int iconSizePixels) {
		return filterTable.sortModel().columnSortOrder(identifier)
						.map(columnSortOrder -> new Arrow(columnSortOrder.sortOrder() == SortOrder.DESCENDING, iconSizePixels, columnSortOrder.priority()))
						.orElse(null);
	}
