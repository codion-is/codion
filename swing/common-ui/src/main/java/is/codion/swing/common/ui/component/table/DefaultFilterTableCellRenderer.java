...
final class DefaultFilterTableCellRenderer<R, C, T> extends DefaultTableCellRenderer implements FilterTableCellRenderer<T> {
	...
	private static final class Settings<R, C, T> {
		...
		private final Collection<Configurer<R, C>> configurations = new ArrayList<>();
		...
		private Settings(SettingsBuilder<R, C, T> builder) {
			...
			if (filterIndicator) {
				configurations.add(new FilterShader<>());
			}
			this.configurations.addAll(builder.configurers);
		...
		private void configure(FilterTable<R, C> filterTable, JComponent component, T value,
			...
			for (Configurer<R, C> configurer : configurations) {
	...
	private static final class FilterShader<R, C> implements Configurer<R, C> {
		...
		private boolean filterEnabled(C identifier, FilterTableModel<?, C> tableModel) {
			if (filterEnabledSet) {
	...
	static final class DefaultBuilder<R, C, T> implements Builder<R, C, T> {
		...
		@Override
		public Builder<R, C, T> configurer(Configurer<R, C> configurer) {
