...
import is.codion.swing.framework.ui.EntityEditPanel.AttributeDefinitionComparator;
...
public class EntityTablePanel extends JPanel {
	...
	public static final class ControlKeys {
		 ...
		 * @see SwingFilterTableModel.Items#clear()
		 */
	...
	private void configureExcludedColumns() {
		if (configuration.excludeHiddenColumns) {
			ValueSet<Attribute<?>> excluded = model.query().attributes().excluded();
			table.columns().hidden().addConsumer(excluded::set);
			excluded.set(table.columns().hidden().get());
		}
	}
	...
	private static final class EntitySummaryValuesFactory implements SummaryValues.Factory<Attribute<?>> {
		...
		private final EntityDefinition entityDefinition;
		private final SwingFilterTableModel<?, Attribute<?>> tableModel;

		private EntitySummaryValuesFactory(EntityDefinition entityDefinition, SwingFilterTableModel<?, Attribute<?>> tableModel) {
