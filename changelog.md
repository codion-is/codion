...
- SQLServerDatabase.autoIncrementQuery() now throws UnsupportedOperationException, @@IDENTITY returning the value generated by a trigger, Generator.identity() being the alternative.
...
### is.codion.common.db
- SessionContext.prepare() and release(), parameters reordered.
...
- Layouts.flexibleGridLayout() added, returning builder initialized with the default gaps.
### is.codion.swing.framework.model
...
- ValueLink, link cycle detection and validator bridging now key on BaseValue rather than AbstractValue, which ValueList and ValueSet do not extend. Linking two collection values used to get neither: a cycle went undetected, and a value the far end rejected was written to the near end before the far end refused it, leaving the pair diverged.
- ValueLink.LinkedValidator now bridges through the public Value.validate() gate for a Value implemented outside the framework, whose validators it can not reach into. Linking one used to skip its validators entirely, so a value it rejected was written to the other end before it refused, leaving the pair diverged. Cycle detection still reaches only as far as the framework's own link bookkeeping, which Value.link() now says.
- ValueLink.LinkedValidator now guards against re-entrancy. Validating walks the connected link graph, and excluding the immediate partner unwinds a chain but not a value linked to two others sharing an origin, where a diamond used to recurse until the stack gave out.
- ValueLink.preventLinkCycle now carries a visited set, so a diamond in the link graph is walked once rather than once per path reaching it.
- ValueSet.set(Set) and ValueList.set(List) now take a snapshot like every other mutation. Value.set(C) is the more specific overload whenever the declared type is passed, so it landed on the raw path instead of ValueCollection.set(Collection): the caller's collection was stored as it stood, leaving the value's contents mutable from the outside and able to change behind its back, with no validation and no notification, and handing observers a snapshot that was not one.
### is.codion.common.utilities
...
- AbstractEntityEditor.Settings, the insert, update, delete and read-only guards now name the entity type they refused, a master's persist running them on every present detail editor too.
### is.codion.dbms.h2
...
- LoginDialogBuilder.validationProgressDelay() added along with VALIDATION_PROGRESS_BAR_DELAY configuration value.
### is.codion.swing.framework.ui
...
- FilterTableColumnModel.identifier(modelColumnIndex) removed.
- FilterTableColumnModel.ColumnSelection.identifiers() now reports the identifiers of the selected columns once columns have been hidden or moved.
- FilterTableColumn.setFixedWidth() renamed fixedWidth().
...
- EntityTablePanel.REQUEST_TABLE_FOCUS renamed FOCUS_TABLE.
- EntityPanel.REQUEST_EDIT_PANEL_FOCUS renamed FOCUS_EDIT_PANEL.
...
- DefaultRemoteEntityConnectionProvider.RemoteEntityConnectionHandler.invoke() refactored.
...
- DefaultRefresher now provides async refresh via ProgressWorker, gated by Dispatcher, async by default respecting FilterModel.ASYNC.
- FilterModel.AbstractRefresher removed, DefaultRefresher is now the single Refresher implementation.
- FilterModel.Refresher.builder() added, replacing the FilterModel.refresher() factory method.
...
- AbstractRefreshWorker removed, folded into common-model DefaultRefresher.
### is.codion.swing.common.ui
...
- AttributeDefinition.DEFAULT_FRACTION_DIGITS removed.
- EntityDefinition.Builder.AttributeStep added.
### is.codion.framework.model
...
- EditorLink.Builder R type parameter removed.
### is.codion.swing.framework.ui
...
### is.codion.swing.common.model
- ProgressWorker.Handler.onException() bug fixed, default implementation now rethrows instead of swallowing.
### is.codion.swing.common.ui
...
- EntityEditPanel.DeleteCommand.Builder.requestFocus() added.
...
- ProgressWorker bug fixed, now propagates exceptions thrown from an onStarted handler, instead of hanging.
### is.codion.swing.common.ui
...
- EntityEditor type parameters removed.
### is.codion.swing.framework.model
...
- AbstractEntityModel no longer abstract, renamed DefaultEntityModel.
### is.codion.swing.framework.model
