...
The {url-javadoc}/org/jminor/framework/domain/StringProvider.html[StringProvider] class is for providing __toString()__ implementations for entities. This value is f. ex. used when entities are displayed in a ComboBox or as a foreign key values in table views.
