...
Finally we bind a couple events, for further information see <<{manual-dir}/jminor-manual.adoc#Event-binding, Manual#Event binding>>.
...
We create a main application panel by extending {javadoc-root}/org/jminor/framework/client/ui/EntityApplicationPanel.html[EntityApplicationPanel]. Overriding *setupEntityPanelProviders()* we configure two *EntityPanelProvider* objects, __employeePanelProvider__ and __departmentPanelProvider__. We'll define the *EmployeePanelProvider* class later.
