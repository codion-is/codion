...
We start by callin the super class method which creates an unfiltered {url-javadoc}/org/jminor/framework/model/EntityComboBoxModel.html[EntityComboBoxModel], then we configure the one for the manager foreign key so it only display employees with the job PRESIDENT or MANAGER.
...
Finally we bind a couple of events, for further information see <<{dir-manual}/jminor-manual.adoc#Event-binding, Manual#Event binding>>.
...
We create a method for viewing a report, which is called via an action we'll initialize in the next step. For further information about report viewing and printing see <<{dir-manual}/jminor-manual.adoc#Reporting-with-JasperReports, Manual#Reporting with JasperReports>>.
...
We create a main application panel by extending {url-javadoc}/org/jminor/framework/client/ui/EntityApplicationPanel.html[EntityApplicationPanel]. Overriding *setupEntityPanelProviders()* we configure two sets of *SwingEntityModelProvider* and *EntityPanelProvider* objects, which allow lazy initialization of UI components. We'll define the *EmployeePanelProvider* class later. We call *addEntityPanelProvider()* making the department panel the main application panel.
