...
The domain model is based around three classes; {url-javadoc}/org/jminor/framework/domain/entity/Entity.html[Entity], which represents a row in a table, {url-javadoc}/org/jminor/framework/domain/entity/EntityDefinition.html[EntityDefinition] which contains the meta-data for each entity type (properties, data types and such) and {url-javadoc}/org/jminor/framework/domain/property/Property.html[Property], which represents columns. Domain models must extend the
{url-javadoc}/org/jminor/framework/domain/Domain.html[Domain] class, which in turn provides a {url-javadoc}/org/jminor/framework/domain/entity/Entities.html[Entities] instance via {url-javadoc}/org/jminor/framework/domain/Domain.html#getEntities[getEntities()], which serves as a container for the domain entity definitions and as a factory for *Entity* and *Entity.Key* instances.
...
[source,java]
----
include::{dir-world-source}/org/jminor/framework/demos/world/domain/World.java[tags=entityAndPropertyIds]
----
...
----
include::{dir-world-source}/org/jminor/framework/demos/world/domain/World.java[tags=primaryKey]
----
...
[source,java,indent=0]
----
include::{dir-world-source}/org/jminor/framework/demos/world/domain/World.java[tags=compositePrimaryKey]
----
...
[source,java,indent=0]
----
include::{dir-world-source}/org/jminor/framework/demos/world/domain/World.java[tags=columnProperty]
----
...
[source,java,indent=0]
----
include::{dir-world-source}/org/jminor/framework/demos/world/domain/World.java[tags=blobProperty]
----

[#_foreignkeyproperty]
=== ForeignKeyProperty

{url-javadoc}/org/jminor/framework/domain/property/ForeignKeyProperty.html[ForeignKeyProperty] is a wrapper property used to represent a foreign key relation. These foreign keys refer to the primary key of the referenced entity and must be constructed accordingly in case of composite primary keys.

[source,java,indent=0]
----
include::{dir-world-source}/org/jminor/framework/demos/world/domain/World.java[tags=foreignKeyPropertyCapital]
----
...
[source,java,indent=0]
----
include::{dir-world-source}/org/jminor/framework/demos/world/domain/World.java[tags=booleanProperty]
----
...
[source,java,indent=0]
----
include::{dir-world-source}/org/jminor/framework/demos/world/domain/World.java[tags=denormalizedViewProperty]
----
...
{url-javadoc}/org/jminor/framework/domain/property/SubqueryProperty.html[SubqueryProperty] is used to represent a property which gets its value from a subquery returning a single value. Note that in the example below *reference_id* must be available when the query is run, that is, the entity must include that column as a property.

[source,java,indent=0]
----
include::{dir-world-source}/org/jminor/framework/demos/world/domain/World.java[tags=subqueryProperty]
----
...
[source,java,indent=0]
----
include::{dir-world-source}/org/jminor/framework/demos/world/domain/World.java[tags=derivedProperty]
----

[source,java,indent=0]
----
include::{dir-world-source}/org/jminor/framework/demos/world/domain/World.java[tags=derivedPropertyProvider]
----
...
[source,java,indent=0]
----
include::{dir-world-source}/org/jminor/framework/demos/world/domain/World.java[tags=defineCity]
----
...
The framework provides implementations for most commonly used primary key generation strategies, sequence (with or without trigger) and auto-increment columns. The {url-javadoc}/org/jminor/framework/domain/entity/KeyGenerators.html[KeyGenerators] class serves as a factory for {url-javadoc}/org/jminor/framework/domain/entity/KeyGenerator.html[KeyGenerator] implementations. Static imports are assumed in the below examples.
...
[source,java,indent=0]
----
include::{dir-world-source}/org/jminor/framework/demos/world/domain/World.java[tags=sequence]
----
...
The {url-javadoc}/org/jminor/framework/domain/entity/StringProvider.html[StringProvider] class is for providing *toString()* implementations for entities. This value is used wherever entities are displayed, for example in a ComboBox or as foreign key values in table views.
...
{url-javadoc}/org/jminor/framework/domain/entity/ColorProvider.html[ColorProvider] is used to provide colors for entity properties, used as background color in table cells for example.

[source,java,indent=0]
----
include::{dir-world-source}/org/jminor/framework/demos/world/domain/World.java[tags=colorProvider]
----

== Validation

Custom validation of Entities is performed by implementing a {url-javadoc}/org/jminor/framework/domain/entity/EntityValidator.html[EntityValidator].
...
[source,java,indent=0]
----
include::{dir-world-source}/org/jminor/framework/demos/world/domain/World.java[tags=validator]
----
...
[source,java,indent=0]
----
include::{dir-petstore-source}/org/jminor/framework/demos/petstore/manual/EntitiesInAction.java[tags=entitiesInAction]
----
...
To unit test the CRUD operations on the domain model extend {url-javadoc}/org/jminor/framework/domain/entity/test/EntityTestUnit.html[EntityTestUnit].
...
* {url-javadoc}/org/jminor/framework/domain/entity/test/EntityTestUnit.html#initializeReferenceEntity-java.lang.String-java.util.Map-[initializeReferenceEntity] should return an instance of the given entity type to use for a foreign key reference required for inserting the entity being tested.
* {url-javadoc}/org/jminor/framework/domain/entity/test/EntityTestUnit.html#initializeTestEntity-java.lang.String-java.util.Map-[initializeTestEntity] should return a entity to use as basis for the unit test, that is, the entity that should be inserted, selected, updated and finally deleted.
* {url-javadoc}/org/jminor/framework/domain/entity/test/EntityTestUnit.html#modifyEntity-org.jminor.framework.domain.entity.Entity-java.util.Map-[modifyEntity] should simply leave the entity in a modified state so that it can be used for update test, since the database layer throws an exception if an unmodified entity is updated. If *modifyEntity* returns an unmodified entity, the update test is skipped.
...
[source,java]
----
include::{dir-source}/org/jminor/framework/demos/manual/store/domain/StoreTest.java[tags=storeTest]
----
