...
The domain model is based around three classes; {url-javadoc}/org/jminor/framework/domain/Entity.html[Entity], which represents a row in a table, {url-javadoc}/org/jminor/framework/domain/EntityDefinition.html[EntityDefinition] which contains the meta-data for each entity type (properties, data types and such) and {url-javadoc}/org/jminor/framework/domain/property/Property.html[Property], which represents columns. Domain models must extend the
{url-javadoc}/org/jminor/framework/domain/Domain.html[Domain] class, which serves as a container for the domain entity definitions and as a factory for *Entity* and *Entity.Key* instances.
...
The framework provides implementations for most commonly used primary key generation strategies, sequence (with or without trigger) and auto-increment columns. The {url-javadoc}/org/jminor/framework/domain/KeyGenerators.html[KeyGenerators] class serves as a factory for {url-javadoc}/org/jminor/framework/domain/KeyGenerator.html[KeyGenerator] implementations. Static imports are assumed in the below examples.
...
You can provide a custom key generator strategy by implementing a {url-javadoc}/org/jminor/framework/domain/KeyGenerator.html[KeyGenerator].
...
The {url-javadoc}/org/jminor/framework/domain/StringProvider.html[StringProvider] class is for providing *toString()* implementations for entities. This value is for example used when entities are displayed in a ComboBox or as a foreign key values in table views.
...
{url-javadoc}/org/jminor/framework/domain/Entity.ColorProvider.html[Entity.ColorProvider] is used to provide colors for entity properties, used as background color in table cells for example.
...
Custom validation of Entities is performed by implementing a {url-javadoc}/org/jminor/framework/domain/Entity.Validator.html[Entity.Validator].
...
* {url-javadoc}/org/jminor/framework/domain/test/EntityTestUnit.html#initializeReferenceEntity-java.lang.String-java.util.Map-[initializeReferenceEntity] should return an instance of the given entity type to use for a foreign key reference required for inserting the entity being tested.
