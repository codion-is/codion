...
The JMinor server does not perform any user authentication by default, it leaves that up the the underlying database. An authentication layer can be added by implementing a {url-javadoc}/org/jminor/common/rmi/server/LoginProxy.html[LoginProxy] and setting the following system property.
...
A {url-javadoc}/org/jminor/common/rmi/server/SerializationWhitelist.html[serialization whitelist] can be used by setting the following system property.
...
[source]
----
include::{dir-manual-source}/org/jminor/framework/demos/manual/store/ClientServer.java[tags=runServer]
----

=== HTTP
[source]
----
include::{dir-manual-source}/org/jminor/framework/demos/manual/store/ClientServer.java[tags=runServerWithHttp]
----